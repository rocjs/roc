...
import execute from 'roc-execute/lib/execute';
import getAbsolutePath from 'roc-utils/lib/getAbsolutePath';
import getSuggestions from 'roc-utils/lib/getSuggestions';
import log from 'roc-logger/default';
import merge from 'roc-utils/lib/merge';
import { setContext } from 'roc-context';

import { appendSettings, getSettings } from '../config/manageSettings';
import { setConfig } from '../config/manageConfig';
import addRaw from '../config/addRaw';
import initContext from '../context/initContext';
import buildDocumentationObject from '../docs/buildDocumentationObject';
...
export default function runCli({
    ...
    if (input.coreOptions.version || input.coreOptions.v) {
        ...
        return log.small.log(`${info.package || info.name || 'Unknown'}: ${info.version}  roc-core: ${version}`);
    }
    ...
    if (isString(commandData)) {
        return log.small.log(commandData);
    }
    ...
    runHook('roc-core')('update-settings', () => getSettings())(
